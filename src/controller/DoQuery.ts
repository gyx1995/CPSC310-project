...
//    private courseName: any = {};
    ...
    public query(query: IQueryRequest): Promise<any> {
        ...
        const that: any = this;
        const id = that.findID(query["WHERE"]);
        Log.trace("!!!!!!!!!!!id " + id );
    ...
    public isValid(query: IQueryRequest): number {
        ...
        if ("TRANSFORMATIONS" in query) {
            ...
            if (!("APPLY" in trans) || !("GROUP" in trans)) {
                Log.trace(" transformation missing something");
                return 400;
            }
        }
    ...
    public isKeyValid(query: IQueryRequest): number {
        const dataID = this.findID(query["WHERE"]);
        const options: any = query["OPTIONS"];
            ...
            for (const a of apply) {
                if ((Object.keys(a)[0]).indexOf("_") === -1) {
                    if (applyKeys.indexOf(Object.keys(a)[0]) === -1) {
                        ...
                        Log.trace("the key in APPLY is not unique");
                        return 400;
                    }
                } else {
                    Log.trace("Invalid Apply Keys");
                    return 400;
                }
            }
                ...
                for (const a of apply) {
                    ...
                    if ( "MAX" in v || "MIN" in v || "SUM" in v ||  "AVG" in v) {
                        if (this.courseNkey.indexOf(v2) === -1) {
                            Log.trace("in courses : Max||MIN||SUM||AVG with non Number");
                            return 400;
                        }
                    } else if ( "COUNT" in v) {
                ...
                for (const a of apply) {
                    ...
                    if ( "MAX" in v || "MIN" in v || "SUM" in v ||  "AVG" in v) {
                        if (this.roomNkey.indexOf(v2) === -1) {
                            Log.trace("in rooms : Max||MIN||SUM||AVG with non Number");
                            return 400;
                        }
                    } else if ( "COUNT" in v) {
        ...
        if ("ORDER" in options) {
            ...
            if ((columns.indexOf(order) === -1)) {
                if (!("keys" in order) || !("dir" in order)) {
                    Log.trace("Option order key is wrong");
                    return 400;
                } else {
                    for (const k of order["keys"]) {
                        if (columns.indexOf(k) === -1 ) {
                            Log.trace("Order key: " + k);
                            Log.trace("Columns: " + columns);
                            Log.trace("Order key needs to be included in columns (400)");
                            return 400;
                        }
                    }
                }
